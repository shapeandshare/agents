...
class AbstractCommand(BaseModel):
    ...
    def __init__(self, **kwargs):
        ...
        if self.options is None:
            self.options: CommandOptions = CommandOptions(
                sleep_time=demand_env_var_as_float(name="AGENT_GIT_SLEEP_TIME"),
                retry_count=demand_env_var_as_int(name="AGENT_GIT_RETRY_COUNT"),
                tld=demand_env_var(name="AGENT_GIT_TLD"),
                scheme=demand_env_var(name="AGENT_GIT_SCHEME"),
                timeout=demand_env_var_as_float(name="AGENT_GIT_TIMEOUT"),
            )
    ...
    def _build_requests_params(self, request: WrappedRequest) -> dict:
        ...
        if request.verb in [RequestVerbType.POST, RequestVerbType.PUT]:
    ...
    async def _api_caller(self, request: WrappedRequest, depth: int) -> dict | None:
        ...
        try:
            ...
            elif request.verb == RequestVerbType.PUT:
                response: Response = self.session.put(**params)
